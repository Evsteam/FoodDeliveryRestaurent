...
public class OrderInProgressFragment extends Fragment implements AuthStatusListener, PagedItemClick<OrderModel> {
   ...
   LoaderProgress progress;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(Global.isOnline(getContext())){
            viewModel.apiOrderList(SessionManager.get_user_id(prefe),SessionManager.get_userType(prefe),"1",this);
        }else Global.showDialog(getActivity());

        viewModel.getOrdersList().observe(this, new Observer<List<OrderModel>>() {
            @Override
            public void onChanged(List<OrderModel> orderModels) {
                progress.dismiss();
                if(orderModels.size()>0){
                    notfound.setVisibility(View.GONE);
                    adapter=new OrderInProgressAdapter(getContext(),orderModels,OrderInProgressFragment.this);
