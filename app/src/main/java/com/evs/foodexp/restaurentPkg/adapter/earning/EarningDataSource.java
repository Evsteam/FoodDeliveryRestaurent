...
public class EarningDataSource extends PageKeyedDataSource<Integer, OrderModel> {
    ...
    private static final String SITE_NAME = "foodorderlist";
    private AuthStatusListener authListener;
    private String userId,userType,status,dateBy;


    EarningDataSource(String userId,String userType,String status,String dateBy, AuthStatusListener authListener) {
        this.userId = userId;
        this.status = status;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull final LoadInitialCallback<Integer, OrderModel> callback) {
        ...
        RetrofitApiService.getApiService()
                .getEarning(SITE_NAME, userId,userType,status,dateBy, String.valueOf(FIRST_PAGE))
    ...
    public void loadBefore(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, OrderModel> callback) {
        ...
        RetrofitApiService.getApiService()
                .getEarning(SITE_NAME, userId,userType,status,dateBy, String.valueOf(params.key))
                .enqueue(new Callback<ListResponse<OrderModel>>() {
                    @Override
                    public void onResponse(Call<ListResponse<OrderModel>> call, Response<ListResponse<OrderModel>> response) {
                        if (response.isSuccessful()) {
