...
public class SpecialOrderWeeklyEarningFragment extends Fragment implements AuthStatusListener, AuthMsgListener {
    Button btn_cashOut;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (Global.isOnline(getContext())) {
            viewModel.executeEarning("specialearning",SessionManager.get_user_id(prefs), SessionManager.get_userType(prefs), "Weekly", this);
            viewModel.apiTogetEarningWeekly("specialrequestlist",SessionManager.get_user_id(prefs), "2", SessionManager.get_userType(prefs), "Weekly", this);
        } else Global.showDialog(getActivity());

        adapter = new TogetEarningPagedAdapter(getContext());

        viewModel.getTogetWeeklyEarnigs().observe(this, new Observer<PagedList<GoGetModel>>() {
            @Override
