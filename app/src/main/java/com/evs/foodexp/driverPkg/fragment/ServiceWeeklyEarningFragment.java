...
import com.evs.foodexp.driverPkg.adapter.service_earning.ServiceWeeklyEarningPagedAdapter;
...
public class ServiceWeeklyEarningFragment extends Fragment implements AuthStatusListener, AuthMsgListener {
    Button btn_cashOut;
    private SharedPreferences prefs;
    RecyclerView recylerView;
    LinearLayoutManager layoutManager;
    TextView notfound,restEarnings,restOrderDeliveredNumber,orderDeliveredTxt;
    LoaderProgress progress;
    LinearLayout earningDetails;
    RestorentViewModel viewModel;
    ServiceWeeklyEarningPagedAdapter adapter;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        orderDeliveredTxt = view.findViewById(R.id.orderDeliveredTxt);
        ...
        if (Global.isOnline(getContext())) {
            viewModel.executeEarning("getearning",SessionManager.get_user_id(prefs), SessionManager.get_userType(prefs), "Weekly", this);
            viewModel.apiServicEarningWeekly(SessionManager.get_user_id(prefs), "2", SessionManager.get_userType(prefs), "Weekly", this);
        } else Global.showDialog(getActivity());

        adapter = new ServiceWeeklyEarningPagedAdapter(getContext());

        viewModel.getServiceWeeklyEarnigs().observe(this, new Observer<PagedList<BookingModel>>() {
            @Override
            public void onChanged(PagedList<BookingModel> reviewsModels) {
//                progress.dismiss();
    ...
    public void onSuccessMsg(MsgResponse msgResponce) {
        ...
        if(Integer.parseInt(msgResponce.getMsg())>0){
            ...
            orderDeliveredTxt.setText("Service Completed");
        }else {
