...
import com.evs.foodexp.driverPkg.adapter.service_earning.ServiceEarningPagedAdapter;
...
public class ServiceTodayEarningFragment extends Fragment implements AuthStatusListener, AuthMsgListener {
    ...
    ServiceEarningPagedAdapter adapter;
//    Button cashoutBtn;
...
//    }
        ...
        if (Global.isOnline(getContext())) {
            viewModel.executeEarning("getearning",SessionManager.get_user_id(prefs), SessionManager.get_userType(prefs), "Today", this);
            viewModel.apiServicEarning(SessionManager.get_user_id(prefs), "2", SessionManager.get_userType(prefs), "Today", this);
        } else Global.showDialog(getActivity());

        adapter = new ServiceEarningPagedAdapter(getContext());

        viewModel.getServiceEarnigs().observe(this, new Observer<PagedList<BookingModel>>() {
            @Override
